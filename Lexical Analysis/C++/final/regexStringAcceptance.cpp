...
set <int> epClosure(int currentState, const vector <nfaNode>& NFA)
{
    ...
    // Add the initial state to the queue and closure
    ...
    while (!stateQueue.empty())
    {
        ...
        if (epsilonTransition != NFA[currentState].transition.end()) 
        {
            ...
            {
                // If the state is not in the closure, add it to the queue and closure
                if (closure.find(trans) == closure.end()) 
                {
                    stateQueue.push(trans);
    ...
    for (const nfaNode& state : eNFA.states)
    {
        ...
        for (char symbol : {'a', 'b'})
        {
            ...
            for (int value : first)
            {
                ...
                if (!temp.empty())
                    for (int t : temp)
                        second.insert(epClosure(t, eNFA.states).begin(), epClosure(t, eNFA.states).end());
            }

            // assign it to the state of NFA
            NFA.states[state.state].transition[symbol] = second;
...
dfa nfaToDFA(nfa NFA)
{
    ...
    visited.insert({NFA.initialState});
