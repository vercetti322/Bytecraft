...
nfa eNfaToNfa(const nfa &eNFA)
{
    ...
    for (int i = 0; i < NFA.states.size(); i++)
    {
        ...
        // Update the acceptance property based on epsilon closure
        for (int closureState : currClosure)
        {
            if (eNFA.states[closureState].acceptance)
            {
                NFA.states[i].acceptance = true;
                break;
            }
        }
